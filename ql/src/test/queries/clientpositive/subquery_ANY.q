...
explain select count(*) from part where p_partkey = ANY (select p_partkey from part);
...
-- SOME is same as ANY
explain select count(*) from part where p_partkey = SOME(select min(p_partkey) from part);
select count(*) from part where p_partkey = SOME(select min(p_partkey) from part);

-- ANY with aggregate in subquery
explain select count(*) from part where p_size < ANY (select max(p_size) from part group by p_partkey);
...
-- ANY in having
explain select count(*) from part having count(*) > ANY (select count(*) from part group by p_partkey);
...
--nested
explain select count(*) from part where p_partkey 
