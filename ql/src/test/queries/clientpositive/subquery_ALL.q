...
explain select count(*) from part where p_size < ALL (select max(p_size) from part group by p_partkey);
...
-- ALL in having
explain select count(*) from part having count(*) > ALL (select count(*) from part group by p_partkey);
