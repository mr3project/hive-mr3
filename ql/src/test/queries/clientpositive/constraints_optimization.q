...
explain select c_customer_sk from
...
-- UNION
explain select i from (select i, j from t1 union all select i,j from t2) subq group by i,j;
select i from (select i, j from t1 union all select i,j from t2) subq group by i,j;

-- INTERSECT
explain select i from (select i, j from t1 intersect select i,j from t2) subq group by i,j;
