 ...
 *     key</pre>
 */
public class LeveldbTimelineStore implements TimelineStore {
  ...
  private static final int CURRENT_VERSION_INFO = 1;
  ...
  public void initialize(Configuration conf) throws Exception {
    ...
    db = openWithRepair(factory, dbFile, options);
  ...
  /**
   * Open the LevelDB database. If open fails due to DB corruption
   * (e.g. missing SST files), try LevelDB repair once and reopen.
   * If repair or reopen fails, propagate the exception.
   */
  private DB openWithRepair(JniDBFactory factory, File dbFile, Options options)
      throws IOException {
    try {
      return factory.open(dbFile, options);
    } catch (NativeDB.DBException e) {
      if (!isRepairableOpenFailure(e)) {
        throw e;
      }

      LOG.warn("Failed to open LevelDB timeline store at " + dbFile +
          " due to corruption. Trying repair.", e);

      try {
        factory.repair(dbFile, options);
      } catch (Exception repairException) {
        IOException ioe = new IOException(
            "Failed to repair LevelDB timeline store at " + dbFile,
            repairException);
        ioe.addSuppressed(e);
        throw ioe;
      }
      ...
      try {
        DB repairedDb = factory.open(dbFile, options);
        LOG.warn("Successfully repaired and reopened LevelDB timeline store at " +
            dbFile + ". Some transient timeline data may have been lost.");
        return repairedDb;
      } catch (Exception reopenException) {
        IOException ioe = new IOException(
            "Repaired LevelDB timeline store at " + dbFile +
            " but failed to reopen it",
            reopenException);
        ioe.addSuppressed(e);
        throw ioe;
      }
  ...
  /**
   * Decide whether an open failure is the sort of corruption for which
   * LevelDB repair is worth attempting.
   */
  private static boolean isRepairableOpenFailure(Throwable t) {
    Throwable cur = t;
    while (cur != null) {
    ...
    public void run() {
      while (true) {
        ...
        try {
          ...
          LOG.error("Deletion thread failed", e);
        } catch (InterruptedException e) {
  ...
  /**
   * Load the version (4-byte big-endian int) from LevelDB. If missing,
   * returns CURRENT_VERSION_INFO.
   */
  private int loadVersion() throws IOException {
    try {
      ...
      if (data.length != 4) {
        throw new IOException("Invalid version format, expected 4 bytes");
      }
  ...
  /**
   * Store the version as a 4-byte big-endian int under TIMELINE_STORE_VERSION_KEY.
   */
  private void dbStoreVersion(int version) throws IOException {
    byte[] data = new byte[] {
      (byte)(version >> 24),
      (byte)(version >> 16),
      (byte)(version >> 8),
    ...
    try {
      db.put(bytes(TIMELINE_STORE_VERSION_KEY), data);
    } catch (DBException e) {
      throw new IOException(e);
    }
  }
