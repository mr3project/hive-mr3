...
import com.datamonad.mr3.timeline.security.PamAuthenticator;
import com.datamonad.mr3.timeline.security.PamConstraint;
import com.datamonad.mr3.timeline.security.PamConstraintMapping;
import com.datamonad.mr3.timeline.security.PamLoginService;
...
public class HttpServer {
  ...
  private static final String SSL_PROTOCOL_BLACKLIST = "SSLv2,SSLv3";
  ...
  public static class Builder {
    ...
    private String contextRootRewriteTarget = "/index.html";  // required for serving MR3-UI JavaScript
    private boolean xFrameEnabled;
    private XFrameOption xFrameOption = XFrameOption.SAMEORIGIN;

    private ServletHolder jerseyAtsHolder, jerseyAmProxyHolder, jerseyServerHolder;
    private String mountAtsPath, mountAmProxyPath, mountServerPath;
    ...
    public void setJerseyAtsMount(ServletHolder jerseyAtsHolder, String mountAtsPath) {
      this.jerseyAtsHolder = jerseyAtsHolder;
      this.mountAtsPath = mountAtsPath;
    }

    public void setJerseyAmProxyMount(ServletHolder jerseyAmProxyHolder, String mountAmProxyPath) {
      this.jerseyAmProxyHolder = jerseyAmProxyHolder;
      this.mountAmProxyPath = mountAmProxyPath;
    }

    public void setJerseyServerMount(ServletHolder jerseyServerHolder, String mountServerPath) {
      this.jerseyServerHolder = jerseyServerHolder;
      this.mountServerPath = mountServerPath;
    }
  }
  ...
  WebAppContext createWebAppContext(Builder b) {
    ...
    ctx.setContextPath("/");
    // to serve configs.env
    ctx.getMimeTypes().addMimeMapping("env", "application/javascript");

    // Mount Jersey ServletHolder before the DefaultServlet
    if (b.jerseyAtsHolder != null) {
      ctx.addServlet(b.jerseyAtsHolder, b.mountAtsPath);
    }
    if (b.jerseyAmProxyHolder != null) {
      ctx.addServlet(b.jerseyAmProxyHolder, b.mountAmProxyPath);
    }
    if (b.jerseyServerHolder != null) {
      ctx.addServlet(b.jerseyServerHolder, b.mountServerPath);
    }

    // Serve static resources directly from the classpath
    // Assumes UI files are packaged under /webapps/<appName> in the JAR
    String resourcePath = "/webapps/" + b.name;
    ctx.setBaseResource(Resource.newClassPathResource(resourcePath));

    // Mount the DefaultServlet to serve index.html, assets, etc.
  ...
  void setupSpnegoFilter(Builder b, ServletContextHandler ctx) throws IOException {
    ...
    // TODO: use pathSpec = "/ws/*" to protect Timeline server only
    ...
    if (!b.useSSL) {
      ...
      Set<String> excludedSSLProtocols = Sets.newHashSet(
        Splitter.on(",").trimResults().omitEmptyStrings().split(
          Strings.nullToEmpty(SSL_PROTOCOL_BLACKLIST)));
  ...
  private void createWebServer(final Builder b) throws IOException {
    ...
    int queueSize = threadPool.getMaxThreads();
    ...
    // 2) for any path that
    //    - does NOT start with /ats, /proxy, /server
    //    - and does NOT contain a dot (.)  -> i.e., not a static file
    RewriteRegexRule spaRule = new RewriteRegexRule();
    spaRule.setRegex("^(?!/(ats|proxy|server)/)(?!.*\\..*).*$");
    spaRule.setReplacement(b.contextRootRewriteTarget);
    spaRule.setTerminating(true);
    rwHandler.addRule(spaRule);
