...
public class HiveSplitGenerator extends InputInitializer {
    ...
    try {
      ...
      if (groupingEnabled) {
        ...
        if (generateSingleSplit &&
            HiveConf.getVar(conf, HiveConf.ConfVars.HIVE_TEZ_INPUT_FORMAT).equals(HiveInputFormat.class.getName())) {
  ...
  List<Event> createEventList(boolean sendSerializedEvents, InputSplitInfoMem inputSplitInfo)
    ...
    if (sendSerializedEvents) {
      ...
      // Hive-MR3 does not use HIVE_TEZ_INPUT_FS_SERIALIZATION_THRESHOLD
      int count = 0;
