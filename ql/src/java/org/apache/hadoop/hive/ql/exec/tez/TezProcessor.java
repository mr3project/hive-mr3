...
public class TezProcessor extends AbstractLogicalIOProcessor {
  ...
  public boolean run(Map<String, LogicalInput> inputs, Map<String, LogicalOutput> outputs)
    ...
    synchronized (this) {
      boolean limitReached = LimitOperator.checkLimitReached(queryId, dagIdId, vertexName);
      if (limitReached) {
        LOG.info("TezProcessor exits early as query limit already reached: {}",
    ...
    boolean limitReached = LimitOperator.checkLimitReached(queryId, dagIdId, vertexName);
    if (limitReached) {
      LOG.info("Query limit already reached, asking for soft-kill sibling Tasks: {}",
          processorContext.getUniqueIdentifier());
      return true;
    } else {
      return false;
