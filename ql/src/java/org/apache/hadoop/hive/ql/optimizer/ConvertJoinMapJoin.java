...
public class ConvertJoinMapJoin implements NodeProcessor {
  ...
  public Object
    ...
    String engine = HiveConf.getVar(context.conf, ConfVars.HIVE_EXECUTION_ENGINE);
    ...
    MemoryMonitorInfo memoryMonitorInfo = getMemoryMonitorInfo(maxSize, context.conf, isLlap && isMr3, estimateNumExecutors);
    ...
    if (numBuckets > 1) {
      if (context.conf.getBoolVar(HiveConf.ConfVars.HIVE_CONVERT_JOIN_BUCKET_MAPJOIN_TEZ)) {
        ...
        if (isLlap) {
          if (selectJoinForLlap(context, joinOp, tezBucketJoinProcCtx, mapJoinConversionPos, numBuckets)) {
            return null;
          }
  ...
  private int getEstimateNumExecutors(HiveConf conf) {
    ...
    int mapSize = conf.getIntVar(HiveConf.ConfVars.MR3_MAP_TASK_MEMORY_MB);
    int containerSize;
