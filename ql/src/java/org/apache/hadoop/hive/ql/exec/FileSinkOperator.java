...
public class FileSinkOperator extends TerminalOperator<FileSinkDesc> implements
  ...
  static class QueryResultDagOutputWriter {
    ...
    private void writeRecord(Writable writable) throws IOException, HiveException {
      if (conf.isUsingBatchingSerDe()) {
        enforceMaxBytes(getWritableLength(writable) + Integer.BYTES);
        writeBinaryRecord(writable);
      } else {
        enforceMaxBytes(getWritableLength(writable) + 1L);
        writeTextRecord(writable);
      }
    }

    private long getWritableLength(Writable writable) throws IOException {
      if (writable instanceof Text) {
        return ((Text) writable).getLength();
      }
      if (writable instanceof BytesWritable) {
        return ((BytesWritable) writable).getSize();
      }
    ...
    private void enforceMaxBytes(long recordBytes) throws HiveException {
      if (maxBytes >= 0 && recordBytes > maxBytes - writer.size()) {
