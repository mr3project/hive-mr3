...
public class MR3DistCp extends Configured implements Tool {

  static final Logger LOG = LoggerFactory.getLogger(MR3DistCp.class);
   ...
   * @throws Exception
   */
  public MR3DistCp(HiveConf configuration, DistCpOptions inputOptions)
      throws Exception {
  ...
  public Job createAndSubmitJob() throws Exception {
    ...
    try {
      synchronized(this) {
        ...
        assert metaFolder != null;  // set in MR3DistCp()
    ...
    // since we use MR3, there is no jobID
    return job;
  }

  private TezWork createTezWork(JobConf jobConf) {
    MapReduceMapWork mapWork= new MapReduceMapWork(jobConf, "distcp");
  ...
  private Path createMetaFolderPath() throws Exception {
    MR3Session mr3Session = getMr3Session(hiveConf);
    Path sessionScratchDir = mr3Session.getSessionScratchDir();
    Path stagingDir = new Path(sessionScratchDir, "staging");
  ...
  private MR3Session getMr3Session(HiveConf hiveConf) throws Exception {
    ...
    if (mr3Session == null) {
      LOG.info("Starting MR3 Session for DistCp");
